...
namespace seal
{
                                    ...
                                    const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
      ...
      // TODO: START OF NAF MODIFICATION HERE
      ...
      if (!galois_keys.has_key(steps_to_galois_elt(steps, coeff_count))) {
        ...
        for (size_t i = 0; i < naf_steps.size(); i++)
        {
          ...
          cout << "current NAF " << i << "th step = " << naf_steps[i] << endl;

          // Apply rotation for this step
          rotate_internal(encrypted, naf_steps[i], galois_keys, MemoryManager::GetPool());//move(pool));
        }
      }
      // TODO: END OF NAF MODIFICATION HERE
