...
namespace seal
{
        ...
        const GaloisKeys &galois_keys, MemoryPoolHandle pool)
    {
        ...
        if (!galois_keys.has_key(galois_elt))
        {
            ...
            {
                // Ignore when the step is 0: nothing to do
                if (naf_array[i] == 0) {
                    continue;
                }

                // Record the direction (sign) of the steps
                bool sign = (naf_array[i] < 0);
                int current_step = abs(naf_array[i])*pow(2,i);

                // We use either 3 or -3 as our generator, depending on the sign
                uint64_t two_power_of_gen = exponentiate_uint64(3, current_step);
                two_power_of_gen &= (m - 1);

                if (sign) {
                    current_step = subgroup_size - current_step;
                    try_mod_inverse(exponentiate_uint64(3,current_step), m, two_power_of_gen);
                }
        ...
        // TODO: END NAF MODIFICATION HERE
